...
export default function CategoryGrocery({
  ...
  onSubmitOrder,
  user,
}: CategoryGroceryProps) {
  const isAr = lang === 'ar';

  const [searchQuery, setSearchQuery] = useState('');
  const [selectedStoreId, setSelectedStoreId] = useState('target');
  const [selectedCategory, setSelectedCategory] = useState<string>(
    isAr ? 'مواد غذائية' : 'Groceries'
  );
  ...
  const filteredProducts = GROCERY_ITEMS.filter((prod) => {
    ...
    const matchesStore = prod.storeId === selectedStoreId;
  ...
  const handleCheckout = () => {
    ...
    const newOrder: Partial<Order> = {
      ...
      deliveryAddress: isAr
        ? 'موقعي الحالي (برج الفيصلية)'
        : 'My Current Location (Faisaliah Tower)',
  ...
  return (
    <div
      className="flex flex-col h-full bg-neutral-950 text-white font-sans"
      dir={isAr ? 'rtl' : 'ltr'}
    >
      {/* Header */}
      <div className="bg-neutral-950/90 backdrop-blur-md px-4 py-3 border-b border-neutral-800 flex items-center justify-between sticky top-0 z-30">
        ...
        <div className="text-center flex-1">
          ...
          <p className="text-[10px] text-amber-400">
            {isAr
              ? 'نشتري لك ما تحتاجه ونوصله لبابك'
              : 'We purchase and deliver your shopping lists'}
          </p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto pb-24 px-3 space-y-4 pt-2">
        ...
        <div className="relative mt-3 h-40 w-full rounded-2xl overflow-hidden border border-neutral-800 bg-neutral-950">
          <img
            ...
            alt="Grocery Banner"
            className="w-full h-full object-cover object-center brightness-90 contrast-110 saturate-110"
          />
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder={
              isAr
                ? 'ابحث عن منتج أو متجر تجاري...'
                : 'Search for soap, milk, store item...'
            }
            value={searchQuery}
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center px-1">
            <span className="text-xs font-bold text-neutral-200">
              {isAr ? 'اختر المتجر' : 'Selected Store'}
            </span>
            <span className="text-[10px] text-amber-500 font-semibold cursor-pointer">
          ...
          <div className="flex gap-2.5 overflow-x-auto pb-1 scrollbar-none">
            {STORES.map((st) => (
              <div
                ...
                className={`flex-shrink-0 flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl border cursor-pointer transition-all ${
                  selectedStoreId === st.id
                    ? 'bg-neutral-900 border-amber-400/80 shadow-md shadow-amber-400/5'
              ...
              >
                ...
                <div className="flex flex-col text-left">
                  <span className="text-xs font-bold text-neutral-200">
                    {isAr ? st.name : st.nameEn}
                  </span>
                  <span className="text-[9px] text-amber-400 font-medium font-sans">
                    ★ {st.rating} • {isAr ? st.deliveryTime : st.deliveryTimeEn}
                  </span>
                </div>
              </div>
            ))}
        ...
        <div className="flex gap-1 bg-neutral-900 p-1 rounded-xl">
          {groceryCategories.map((cat) => (
            <button
              ...
              className={`flex-1 py-1.5 rounded-lg text-center text-xs font-medium cursor-pointer transition-all ${
        ...
        <div className="space-y-2.5">
          <div className="px-1 text-[11px] text-neutral-500 uppercase tracking-wider font-semibold">
            {isAr
              ? `منتجات متوفرة في ${activeStore.name}`
              : `Products available in ${activeStore.nameEn}`}
          </div>

          {filteredProducts.length === 0 ? (
            <div className="text-center py-10 bg-neutral-900/50 rounded-2xl border border-neutral-800 text-neutral-500 text-xs">
              {isAr
                ? 'لا توجد نتائج مطابقة، جرب كلمة أخرى!'
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {filteredProducts.map((prod) => {
                ...
                return (
                  <div
                    key={prod.id}
                    className="bg-neutral-900 border border-neutral-800 rounded-xl p-3 flex flex-col justify-between hover:border-neutral-700 transition-all group"
                  >
                    <div className="relative">
                      <div className="bg-neutral-950 rounded-lg h-20 flex items-center justify-center mb-2 border border-neutral-800 group-hover:scale-105 duration-300 overflow-hidden">
                        <img
                          src={prod.image}
                    ...
                    <div className="mt-3 flex gap-1.5">
                      {qtyInCart > 0 ? (
                        ...
                        <button
                          onClick={() => handleAddToCart(prod.id)}
                          className="bg-neutral-800 hover:bg-neutral-700 text-neutral-200 border border-neutral-700 px-2 py-1.5 rounded-lg w-full text-[10px] font-bold transition-all flex items-center justify-center gap-1 active:scale-95 cursor-pointer"
                        >
      ...
      {cartItemCount > 0 && (
        <div className="absolute bottom-[58px] inset-x-3 bg-amber-400 rounded-2xl p-3 shadow-2xl z-40 text-black flex items-center justify-between border border-amber-300">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-sm font-black font-mono leading-none">
                ...
                <span className="text-[9px] font-normal opacity-70">
                  {' '}
                  ({isAr ? 'شاملاً التوصيل' : 'incl. delivery fee'})
                </span>
              </span>
            </div>
          </div>
          ...
          >
            ...
            <ArrowRight
              className={`h-3.5 w-3.5 stroke-[2.5] ${isAr ? 'rotate-180' : ''}`}
            />
          </button>
        </div>
      )}
